...
/**
 * Train Import Script - Pure Node.js
 * Usage: node scripts/importTrains.js
 */
  ...
  try {
    ...
    for await (const line of rl) {
      ...
      try {
        ...
        if (!stationRecordsMap.has(record.station_code)) {
          stationRecordsMap.set(record.station_code, record);
        }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const stations = Array.from(stats.uniqueStations).map(code => {
        const stationRecord = stationRecordsMap.get(code);
        const stationName = stationRecord?.station_name || code;
        return {
          code,
          name: stationName,
        };
      ...
      if (stations.length > 0) {
        ...
        console.log(`  ✓ Inserted ${stations.length.toLocaleString()} stations`);
      }
      ...
      if (processedTrains.length > 0) {
        ...
        console.log(`  ✓ Inserted ${processedTrains.length.toLocaleString()} trains`);
      }
      ...
      if (processedStops.length > 0) {
        ...
        console.log(`  ✓ Inserted ${processedStops.length.toLocaleString()} train stops`);
      }
    });
