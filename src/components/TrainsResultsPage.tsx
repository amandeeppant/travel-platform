...
import { ArrowLeftRight, Search, SlidersHorizontal, Clock, CheckCircle2, ArrowLeft } from "lucide-react";
...
export default function TrainsResultsPage() {
  const params = useSearchParams();
  const router = useRouter();

  const fromParam = params.get("from") || "New Delhi";
  const toParam   = params.get("to")   || "Mumbai";
  const dateParam = params.get("date") || DATES[0].full;

  const initialDateIdx = DATES.findIndex((d) => d.full === dateParam);
  const [selectedDate, setSelectedDate] = useState(initialDateIdx >= 0 ? initialDateIdx : 0);
  ...
  const handleSearchAgain = () => {
    const p = new URLSearchParams({ from, to, date: DATES[selectedDate].full });
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/search/trains?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${encodeURIComponent(DATES[selectedDate].full)}`);
        if (!res.ok) return;
        const j = await res.json();
        const arr = j?.data?.trains || j?.data?.result || j?.data || null;
        if (Array.isArray(arr)) setRemoteResults(arr);
      } catch (e) {
    ...
    <section className="px-4 md:px-6 pt-[88px] pb-10 md:pb-16" style={{ maxWidth: "1400px", margin: "0 auto" }}>
      ...
      >
        ...
        <p style={{ fontSize: "14px", color: "#6B7280", marginBottom: "24px" }}>
          {fromParam} → {toParam} &nbsp;·&nbsp; {TRAINS.length} trains found
        </p>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flex: 1, minWidth: "240px" }}>
            <input
              ...
              style={{
                background: "#f9fafb", borderRadius: "10px", padding: "8px 14px",
                fontWeight: 600, fontSize: "14px", color: "#111", flex: 1,
                textAlign: "center", border: "1px solid #e5e7eb", outline: "none",
              }}
            />
            <button
              onClick={swap}
        ...
        <div
          ...
          style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "8px", marginBottom: "24px" }}
        >
          {DATES.map((d, i) => (
            <button
              key={d.full}
        ...
        <div className="flex flex-col lg:flex-row w-full gap-5 items-start">
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "14px" }}>
            {(remoteResults ?? TRAINS).map((train: any, i: number) => (
              <motion.div
                key={train.number}
                ...
                <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
                  <div style={{ flex: 1, minWidth: "260px" }}>
                    <div style={{ marginBottom: "12px" }}>
                      <span style={{ fontSize: "11px", color: "#9ca3af", fontFamily: "monospace" }}>#{train.number}</span>
                      <h3 style={{ fontSize: "17px", fontWeight: 700, color: "#111", lineHeight: 1.2 }}>{train.name}</h3>
                      <span style={{ fontSize: "11px", color: "#6B7280" }}>{train.days}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <div style={{ textAlign: "center" }}>
                        <p style={{ fontSize: "22px", fontWeight: 900, color: "#111", lineHeight: 1 }}>{train.dep}</p>
                        <p style={{ fontSize: "11px", fontWeight: 600, color: "#1677FF", marginTop: "2px" }}>{train.from}</p>
                      ...
                      <div style={{ textAlign: "center" }}>
                        <p style={{ fontSize: "22px", fontWeight: 900, color: "#111", lineHeight: 1 }}>{train.arr}</p>
                        <p style={{ fontSize: "11px", fontWeight: 600, color: "#1677FF", marginTop: "2px" }}>{train.to}</p>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "flex-end" }}>
                    {train.classes.map((cls: { cls: string; fare: string; status: string; avail: boolean }) => (
                      <motion.button
                        key={cls.cls}
